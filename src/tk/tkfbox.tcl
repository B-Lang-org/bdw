...
proc ::tk::dialog::file:: {type args} {
	...
	wm transient $w $data(-parent)
        if {$data(-minsize) != ""} {
            wm minsize $w [lindex $data(-minsize) 0] [lindex $data(-minsize) 1]
        }
    }
...
rSASvJTGhnhcV3EJlo3kh53ltF5nAhQAOw==}]
	...
	# action.
	tk_messageBox -type ok -parent $w -icon error -minsize "320 100" \
           -message [mc "Cannot change to the directory\
...
proc ::tk::dialog::file::VerifyFileName {w filename} {
	...
	FILE {
	    if {$data(type) eq "open"} {
		tk_messageBox -icon error -minsize "320 100" -type ok \
                    -parent $w -message [mc "File \"%1\$s\"  does not\
                    exist." [file join $path $file]]
		$data(ent) selection range 0 end
		$data(ent) icursor end
	    } else {
		SetPathSilently $w $path
	...
	PATH {
	    tk_messageBox -icon error -minsize "320 100" -type ok -parent $w \
	...
	ERROR {
	    tk_messageBox -type ok -parent $w -icon error -minsize "320 100" \
                -message [mc "Invalid file name \"%1\$s\"." $path]
	    $data(ent) selection range 0 end
	    $data(ent) icursor end
	}
    }
...
proc ::tk::dialog::file::Done {w {selectFilePath ""}} {
	...
	if {($data(type) eq "save") && [file exists $selectFilePath]} {
	    set reply [tk_messageBox -icon warning -minsize "320 100" \
                -type yesno -parent $w -message [mc "File \"%1\$s\" already\
